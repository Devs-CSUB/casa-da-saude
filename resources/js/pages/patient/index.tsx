...
const PatientIndex = ({
    ...
    const fetchData = async (page: number, field: string, order: string, search: string) => {
        await router.get(route("patients.index"), {
            page: page,
            sortField: field,
            sortOrder: order,
            search: search,
    ...
    const columns = useMemo<Column[]>(() => [
        ...
        { title: "CPF", field: "cpf", sortable: true, align: "center" },
        { title: "E-mail", field: "email", sortable: true },
        { title: "Telefone", field: "phone", sortable: true, align: "center" },
        {
            title: "Nascimento",
            field: "birth",
            sortable: true,
            render: (item) => new Intl.DateTimeFormat("pt-BR").format(new Date(item.birth)),
            align: "center"
        },
        {
            title: "Ativo",
            ...
            render: (item) => (
                ...
                >
                    ...
                    <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem onClick={() => router.get(route('patients.edit', item.id))}>
                            <Edit /> Editar
                        </DropdownMenuItem>
    ...
    const presentAlert = (patient: Patient) => {
    ...
    const handleDelete = () => {
        router.delete(route("patients.destroy", selectedPatient?.id), {
            ...
            preserveScroll: true,
            onSuccess: () => {
                toastSuccess("Sucesso", "Paciente deletado!");
    ...
    return (
        <AppLayout>
            ...
            <div className="px-4 py-6">
                <Heading title="Pacientes" description="Gerencie os pacientes cadastrados" />
                ...
                <AlertConfirm
                    title="Confirmação"
                    message={`Você realmente deseja excluir o paciente "${selectedPatient?.name}"?`}
                    open={openAlert}
